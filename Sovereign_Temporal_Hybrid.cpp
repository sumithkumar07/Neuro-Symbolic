...
public:
    ...
    SovereignTemporalHybrid(int vsize, int hsize, int npages, std::mt19937& gen)
    ...
    void forward_step(const Matrix& x, const Matrix& h_prev, const std::vector<HDVector>& hdc_pages, 
        ...
        for(int i=0; i<DIM; ++i) {
            for(int j=0; j<hidden_size; ++j) poly_hdc.at(0, j) += extracted_hdc[i] * W_hdc_act.at(i, j);
        }

        // 2. TERNARY GRU ALGEBRA
        for (int j = 0; j < hidden_size; ++j) {
            double z_val = b_z.at(0, j); double r_val = b_r.at(0, j); double h_t_val = b_h.at(0, j);
            
            for (int i = 0; i < vocab_size; ++i) {
                z_val += x.at(0, i) * Wz_x_act.at(i, j);
                r_val += x.at(0, i) * Wr_x_act.at(i, j);
                h_t_val += x.at(0, i) * Wh_x_act.at(i, j);
            }
            for (int i = 0; i < hidden_size; ++i) {
                z_val += h_prev.at(0, i) * Wz_h_act.at(i, j);
                r_val += h_prev.at(0, i) * Wr_h_act.at(i, j);
            }
            
            z.at(0, j) = sigmoid(z_val);
            r.at(0, j) = sigmoid(r_val);
            
            // Candidate Intuition includes specific reset memory AND associative boolean multi-page HDC arrays
            for (int i = 0; i < hidden_size; ++i) {
                h_t_val += (r.at(0, i) * h_prev.at(0, i)) * Wh_h_act.at(i, j);
            }
            // Memory Injection
            h_t_val += poly_hdc.at(0, j);
            
            h_tilde.at(0, j) = std::tanh(h_t_val);
            
            // The Mathematical Shield: (1-z)*h_prev allows calculus to flow without vanishing
    ...
    double train_temporal_sequence(const std::vector<int>& seq_idx, const std::vector<std::vector<HDVector>>& hdc_signals, int target_idx, double lr) {
        prepare_forward();
        ...
        // UNROLLING DEEP GRU BACKPROPAGATION CHAIN-RULE
        for (int t = seq_len - 1; t >= 0; --t) {
            Matrix dh(1, hidden_size); 
            Matrix dpoly_hdc(1, hidden_size);
            
            for (int i = 0; i < hidden_size; ++i) {
                double dh_i = dh_next.at(0, i);
                
                // Track into Highway (Gradient completely preserved!)
                ...
                // Track into Candidate Matrix State
                double dh_tilde_i = dh_i * zs[t].at(0, i);
                double dtanh_pre = dh_tilde_i * (1.0 - h_tildes[t].at(0, i) * h_tildes[t].at(0, i));
                
                db_hT.at(0, i) += dtanh_pre;
                dpoly_hdc.at(0, i) = dtanh_pre; // Route gradient error natively into the Hybrid multi-page Memory Array
                
                for(int v=0; v<vocab_size; ++v) dWh_x.at(v, i) += xs[t].at(0, v) * dtanh_pre;
                for(int j=0; j<hidden_size; ++j) {
                    double r_h_val = rs[t].at(0, j) * hs[t].at(0, j);
                    dWh_h.at(j, i) += r_h_val * dtanh_pre;
                    
                    double dr_h_part = dtanh_pre * Wh_h_act.at(j, i);
                    dh.at(0, j) += dr_h_part * rs[t].at(0, j);
                    
                    // Track into Reset Matrix State
                    ...
                    for(int k=0; k<hidden_size; ++k) {
                        dWr_h.at(k, j) += hs[t].at(0, k) * dr_pre;
                        dh.at(0, k) += dr_pre * Wr_h_act.at(k, j);
                    }
                }
            }
            
            // --- BACKPROP INTO SHARD PAGES (From Candidate Matrix gradient drop) ---
            ...
            for(int p=0; p<num_pages; ++p) {
                ...
                for(int j=0; j<hidden_size; ++j) {
                    dW_page.at(j, p) += hs[t].at(0, j) * dlogit; dh.at(0, j) += dlogit * W_page.at(j, p);
                }
            }
            
            for(int k=0; k<DIM; ++k) {
                for(int i=0; i<hidden_size; ++i) dW_hdc.at(k, i) += extracted_hdcs[t][k] * dpoly_hdc.at(0, i);
            }
...
int main() {
    std::cout << "--- PHASE 12: TEMPORAL HARDENING (TERNARY GRU PHYSICS) ---\n";
    std::string data = "";
    ...
    std::cout << "[SYSTEM]: Bounding RNN structure inside Ternary Graceful Decays (GRU). Commencing Math Matrix Overhaul...\n\n";
    SovereignTemporalHybrid engine(vocab_size, 64, num_pages, gen);
    double lr = 0.05; 
    
    for (int epoch = 0; epoch < 250; ++epoch) {
        double total_loss = 0;
        for (int i = 0; i < train_size; ++i) total_loss += engine.train_temporal_sequence(rnn_inputs[i], hdc_inputs[i], rnn_targets[i], lr);
        if (epoch % 50 == 0 || epoch == 249) std::cout << "Epoch " << epoch + 1 << " | Temporal Highway MSE Drop: " << std::fixed << std::setprecision(6) << (total_loss / train_size) << "\n";
    }
